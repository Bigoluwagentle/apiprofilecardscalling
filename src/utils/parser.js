const COUNTRY_MAP = {
  ...
  sudan: "SD", chad: "TD", congo: "CG", "democratic republic of congo": "CD",
  drc: "CD", benin: "BJ", togo: "TG", "ivory coast": "CI", "côte d'ivoire": "CI",
  "cote d'ivoire": "CI", "burkina faso": "BF", guinea: "GN", gabon: "GA",
  namibia: "NA", botswana: "BW", lesotho: "LS", swaziland: "SZ", eswatini: "SZ",
  eritrea: "ER", djibouti: "DJ", mauritius: "MU", seychelles: "SC",
  "sierra leone": "SL", liberia: "LR", gambia: "GM", "guinea-bissau": "GW",
  "cape verde": "CV", "sao tome": "ST", comoros: "KM", libya: "LY",
  tunisia: "TN", algeria: "DZ", morocco: "MA", mauritania: "MR",
  usa: "US", "united states": "US", uk: "GB", "united kingdom": "GB",
  france: "FR", germany: "DE", china: "CN", india: "IN", brazil: "BR",
  indonesia: "ID", pakistan: "PK", bangladesh: "BD", russia: "RU",
  mexico: "MX", japan: "JP", philippines: "PH", vietnam: "VN",
};
...
function parseQuery(q) {
  ...
  const fromMatch = text.match(/(?:from|in)\s+([a-z\s'-]+?)(?:\s+(?:above|below|over|under|between|aged?|who|with|and|$))/);
  const countryRaw = fromMatch ? fromMatch[1].trim() : null;

  if (countryRaw && COUNTRY_MAP[countryRaw]) {
    filters.country_id = COUNTRY_MAP[countryRaw];
  } else {
